...
void ASTBuilder::PrintList(List* tokens)
{
	...
	{
		NewToken* token = static_cast<NewToken*>(tokens->get_accessor(i));
...
{
	char* openingBracket = static_cast<NewToken*>(tokens->get_accessor(openingBracketIndex))->string;
	char* closingBracket;
	...
	else
	{
		...
		throw new Exception("MissingBracket: пропущена скобка.", static_cast<NewToken*>(tokens->get_accessor(openingBracketIndex))->lineIndex);
	}

	for (int count = 0, i = openingBracketIndex; i < tokens->count(); i++)
	{
		char* next = static_cast<NewToken*>(tokens->get_accessor(i))->string;
...
int ASTBuilder::FindOperationWithMinimalPriority(int startPosition, int endPosition)
{
	...
	{
		NewToken* l = static_cast<NewToken*>(tokens->get_accessor(i));
	...
	if (result == -1)
		...
		throw new Exception("InvalidOperation: недопустимая операция.", static_cast<NewToken*>(tokens->get_accessor(startPosition))->lineIndex);
...
void ASTBuilder::Prioritize(List* tokenList)
{
	...
	{
		NewToken* current = static_cast<NewToken*>(tokenList->get_accessor(i));
		NewToken* prev = nullptr;
		if (i > 0)
		{
			prev = static_cast<NewToken*>(tokenList->get_accessor(i - 1));
		}
...
int ASTBuilder::FindToken(int startPosition, int endPosition, char* token)
{
	...
	{
		if (strcmp(static_cast<NewToken*>(tokens->get_accessor(i))->string, token) == 0)
			return i;
	}

	//throw Exceptions::TokenNotFound;
	throw new Exception("TokenNotFound: токен не найден.", static_cast<NewToken*>(tokens->get_accessor(startPosition))->lineIndex);
}
...
List* ASTBuilder::OldTokenToNewToken(List* old)
{
	...
	{
		Automat::ParseResult* oldToken = static_cast<Automat::ParseResult*>(old->get_accessor(i));
		...
		if (oldToken->token == Automat::Token::SysFunction)
		{
			...
			// значение value = 1.
			if (strcmp(oldToken->string, "output") == 0 && strcmp(oldToken->string, "input") == 0)
			{
				value = 1;
			}
		}
...
{
	NewToken* firstElement = (NewToken*)tokens->get_accessor(index);
	...
	if (firstElement->type == Automat::Digit || firstElement->type == Automat::Char)
	{
		...
		{
			const char symbol = firstElement->string[1];
	...
	if (firstElement->type == Automat::UserType)
	{
		...
		if (var == nullptr)
			...
			throw new Exception("VariableNameNotFound: необъявленная переменная или константа.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

		// Переменная хранит ссылку на массив значений.
		if (var->IsReference())
			//throw Exceptions::MissingBracket;
			throw new Exception("MissingBracket: пропущена скобка.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);
	...
	throw new Exception("InvalidCharacter: недопустимый символ.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);
}
...
void ASTBuilder::ParseConstInitialization(int& index)
{
	...
	const int semicolon = FindToken(index, tokens->count(), ";");
	const int lenght = semicolon - index;

	if (lenght != 5 && lenght != 6)
		//throw Exceptions::ConstInitializationError;
		throw new Exception("ConstInitializationError: ошибка инициализации константы.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

	NewToken* type = static_cast<NewToken*>(tokens->get_accessor(++index));	// Тип.

	if (TypeList::Instance().GetTypeIndex(type->string) == -1)
		//throw Exceptions::UnknownType;
		throw new Exception("UnknownType: неподдерживаемый тип данных.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

	NewToken* name = static_cast<NewToken*>(tokens->get_accessor(++index));	// Имя константы.
	...
	if (currentScope->FindInCurrentScope(name->string) != nullptr)
		...
		throw new Exception("RedefinitionVariable: недопустимое переопределение переменной.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

	NewToken* assignment = static_cast<NewToken*>(tokens->get_accessor(++index));

	if (assignment->type != Automat::Assignment)
		//throw Exceptions::UninitializedConstant;
		throw new Exception("UninitializedConstant: неинициализированная константа.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

	int sign = 1; // Знак.

	if (lenght == 6)
	{
		NewToken* signToken = static_cast<NewToken*>(tokens->get_accessor(++index));
	...
	NewToken* value = static_cast<NewToken*>(tokens->get_accessor(++index));	// Значение.
...
void ASTBuilder::ParseInitialization(int& index)
{
	...
	if (semicolon - index < 2)
		...
		throw new Exception("VariableNameIsNotSpecified: не указано имя переменной.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

	// Тип переменной.
	NewToken* type = static_cast<NewToken*>(tokens->get_accessor(index));
	...
	// Имя переменной.
	NewToken* name = static_cast<NewToken*>(tokens->get_accessor(++index));

	if (name->type != Automat::UserType)
		//throw Exceptions::InvalidVariableName;
		throw new Exception("InvalidVariableName: недопустимое имя переменной.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);
	...
	NewToken* bracketOrAssignment = static_cast<NewToken*>(tokens->get_accessor(++index));
	...
	if (bracketOrAssignment->type == Automat::Assignment)
	{
		...
		if (assignment->type != Automat::Assignment)
			//throw Exceptions::InvalidOperation;
			throw new Exception("InvalidOperation: недопустимая операция.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

		// Символьный тип данных.
		if (!strcmp(type->string, "char"))
		{
			char symbol = static_cast<NewToken*>(tokens->get_accessor(++index))->string[1];
	...
	{
		// Размер.
		NewToken* size = static_cast<NewToken*>(tokens->get_accessor(++index));
		int integerSize = 0;
		...
		if (size->type == Automat::Digit)
		{
			if (size->value <= 0 || this->IsFraction(size->string))
				...
				throw new Exception("ArraySizeIsNotAnInteger: размер массива должен быть целым положительным числом.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

			integerSize = static_cast<int>(size->value);
		}
			...
			if (constSize == nullptr)
			{
				...
				else
				{
					if (constSize->IsConst())
					{
						...
						throw new Exception("ArraySizeIsNotAConstant: размер массива должен быть задан константой.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);
				}
			}
			else
			{
				if (constSize->IsConst())
				{
					if (constSize->GetValue()->GetType() == TypeList::Instance().GetTypeIndex("int"))
					{
						integerSize = static_cast<int>(constSize->GetValue()->GetValue());
		...
		if (index != ClosingBracketIndex(index - 2))
			...
			throw new Exception("ArrayInitializationError: ошибка объявления массива.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);
	...
	throw new Exception("InitializationError: ошибка объявления или инициализации.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);
}
...
TNode* ASTBuilder::ParseIfElse(int& index)
{
	...
	// 1. Условие if.
	NewToken* openingParenthesis = static_cast<NewToken*>(tokens->get_accessor(++index));

	if (strcmp(openingParenthesis->string, "(") != 0)
		//throw Exceptions::MissingBracket;
		throw new Exception("MissingBracket: пропущена скобка.", static_cast<NewToken*>(tokens->get_accessor(index))->lineIndex);

	const int closingParenthesis = ClosingBracketIndex(index);

	TNode* condition = ParseLine(index, closingParenthesis);
	index = closingParenthesis;

	// 2. Тело then.
	NewToken* openingBrace = static_cast<NewToken*>(tokens->get_accessor(++index));
	...
	NewToken* elseToken = (NewToken*)tokens->get_accessor(++index);
	TList* elseBranch = nullptr;

	// Если есть else.
	if (elseToken && strcmp(elseToken->string, "else") == 0)
	{
		NewToken* openingBrace = (NewToken*)tokens->get_accessor(++index);
		if (openingBrace->type != Automat::Bracket)
			//throw Exceptions::MissingBracket;
			throw new Exception("MissingBracket: пропущена скобка.", ((NewToken*)tokens->get_accessor(index))->lineIndex);
...
TNode* ASTBuilder::ParseWhile(int& index)
{
	...
	// 1. Условие.
	NewToken* openingParenthesis = (NewToken*)tokens->get_accessor(++index);
...
{
	// Название переменной.
	NewToken* variableName = (NewToken*)tokens->get_accessor(index++);

	TValueKeeper* keeper = currentScope->Find(variableName->string);
	if (keeper == nullptr)
		//throw Exceptions::VariableNameNotFound;
		throw new Exception("VariableNameNotFound: необъявленная переменная или константа.", ((NewToken*)tokens->get_accessor(index))->lineIndex);
	...
	{
		//  Квадратные скобки.
		NewToken* brackets = (NewToken*)tokens->get_accessor(index);
		...
		else
		{
			...
			throw new Exception("IndexIsNotSpecified: не указан индекс элемента массива.", ((NewToken*)tokens->get_accessor(index))->lineIndex);
...
{
	NewToken* token = (NewToken*)tokens->get_accessor(start);
	...
	while (i->type == Automat::Token::Bracket && ClosingBracketIndex(start) == end)
	{
		...
		i = (NewToken*)tokens->get_accessor(start);
	}
	...
	NewToken* element = (NewToken*)tokens->get_accessor(pos);
	...
	if (element->type == Automat::IncOrDec)
	{
		...
		{
			if (((NewToken*)tokens->get_accessor(pos + 1))->type == Automat::UserType)
			{
				// Префиксный.
		...
		throw new Exception("UnknownOperation: неподдерживаемая операция.", ((NewToken*)tokens->get_accessor(start))->lineIndex);
	}

	// 6. Системные функции, возвращающие значение.
	if (element->type == Automat::SysFunction && element->value)
	{
		NewToken* bracket = (NewToken*)tokens->get_accessor(++pos);

		if (bracket->type != Automat::Bracket)
			//throw Exceptions::MissingBracket;
			throw new Exception("MissingBracket: пропущена скобка.", ((NewToken*)tokens->get_accessor(pos))->lineIndex);
		...
		if (closeBracket - pos == 1)
			...
			throw new Exception("MissingArguments: пропущены аргументы при вызове функции.", ((NewToken*)tokens->get_accessor(pos))->lineIndex);
		...
		if (BinaryOperationList::Instance().IsOperation(element->string))
		{
			...
			{
				NewToken* separator = (NewToken*)tokens->get_accessor(i);
		...
		throw new Exception("SystemFunctionIsNotSupported: неподдерживаемая системная функция.", ((NewToken*)tokens->get_accessor(start))->lineIndex);
	}
	...
	if (element->type == Automat::Assignment)
	{
		...
		if (resultType == TNodeType::Const)
			...
			throw new Exception("ConstantReinitialization: изменение значения константы невозможно.", ((NewToken*)tokens->get_accessor(start))->lineIndex);
	...
	//throw Exceptions::InvalidOperation;
	throw new Exception("InvalidOperation: недопустимая операция.", ((NewToken*)tokens->get_accessor(start))->lineIndex);
}
...
void ASTBuilder::ParseMultiLine(int start, int end)
{
	for (int i = start; i < end && i < tokens->count(); i++)
	{
		...
		if (token->type == Automat::Token::Condition)
		{
			...
			{
				//throw Exceptions::InvalidOperation;
				throw new Exception("InvalidOperation: недопустимая операция.", ((NewToken*)tokens->get_accessor(i))->lineIndex);
		...
		if (token->type == Automat::Token::SysFunction && token->value == 0)
		{
			...
			NewToken* bracket = static_cast<NewToken*>(tokens->get_accessor(++i));

			if (bracket->type != Automat::Token::Bracket)
				//throw Exceptions::MissingBracket;
				throw new Exception("MissingBracket: пропущена скобка.", static_cast<NewToken*>(tokens->get_accessor(i))->lineIndex);
			...
			if (closeBracketIndex - i == 1)
				//throw Exceptions::MissingArguments;
				throw new Exception("MissingArguments: пропущены аргументы при вызове функции.", ((NewToken*)tokens->get_accessor(i))->lineIndex);

			i++;

			NewToken* digitOrChar = (NewToken*)tokens->get_accessor(i);
